...
> 1. (보통) 주소록에서 도로명으로 검색하는 함수를 작성해 보라. `findEntry` 함수의 구현을 참고하라. PSCi에서 작성한 함수를 테스트해 보라.
> 1. (보통) `AddressBook`에 어떤 이름이 나타나는지만 확인하는 함수를 작성해 보라. 반환값은 Boolean 값이다. **힌트**: 어떤 리스트가 비어있는지 여부를 확인하는 `Data.List.null` 함수의 타입을 PSCi를 이용하여 알아보라. 
